...
export default function ResponseDisplay({ response, isLoading = false }: ResponseDisplayProps) {
  ...
  const isHTML = typeof response === 'string' && response.trim().startsWith('<');

  // Effect: streaming only for plain text / markdown
  useEffect(() => {
    if (isLoading || isHTML) {
      setDisplayedText('');
      setIsComplete(false);
  ...
  }, [response, isLoading, currentIndex, isHTML]);
